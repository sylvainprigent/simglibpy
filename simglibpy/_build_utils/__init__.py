...
DEFAULT_ROOT = 'simglibpy'


def _check_cython_version():
    message = ('Please install Cython with a version >= {0} in order '
               'to build a simglibpy from source.').format(
                    CYTHON_MIN_VERSION)
    try:
        import Cython
...
def cythonize_extensions(top_path, config):
    ...
    # check simple compilation with OpenMP. If it fails simglibpy will
    ...
    # simglibpy module (only at build-time) to be used twice:
    # - First to set the value of simglibpy_OPENMP_PARALLELISM_ENABLED, the
    ...
    simglibpy._OPENMP_SUPPORTED = check_openmp_support()
    ...
    config.ext_modules = cythonize(
        ...
        nthreads=n_jobs,
        compile_time_env={
            'simglibpy_OPENMP_PARALLELISM_ENABLED': simglibpy._OPENMP_SUPPORTED},
