...
# main simglibpy __init__ can detect if it is being loaded by the setup
# routine, to avoid attempting to load components that aren't built yet:
# the numpy distutils extensions that are used by simglibpy to
# recursively build the compiled extensions in sub-packages is based on the
# Python import machinery.
builtins.__simglibpy_SETUP__ = True


DISTNAME = 'simglibpy'
DESCRIPTION = 'Library for 3D+t image scientific image processing'
...
PROJECT_URLS = {
    ...
    'Documentation': '',
    'Source Code': 'https://github.com/simglib/simglibpy'
}

# We can actually import a restricted version of simglibpy that
# does not need the compiled code
import simglibpy
import simglibpy._build_utils.min_dependencies as min_deps  # noqa


VERSION = simglibpy.__version__
...
class CleanCommand(Clean):
    ...
    def run(self):
        ...
        if os.path.exists('build'):
            shutil.rmtree('build')
        for dirpath, dirnames, filenames in os.walk('simglibpy'):
...
try:
    ...
    class build_ext_subclass(build_ext):
        def build_extensions(self):
            from simglibpy._build_utils.openmp_helpers import get_openmp_flag

            if simglibpy._OPENMP_SUPPORTED:
...
# To automate release of binary packages for simglibpy we need a tool
...
def configuration(parent_package='', top_path=None):
    ...
    from simglibpy._build_utils import _check_cython_version
    ...
    config.add_subpackage('simglibpy')
...
def check_package_status(package, min_version):
    ...
    req_str = "simglibpy requires {} >= {}.\n".format(
        package, min_version)

    instructions = ("Installation instructions are available on the "
                    "simglibpy website: "
                    "http://simglibpy.org/stable/install.html\n")
...
def setup_package():
    ...
    if len(sys.argv) == 1 or (
        ...
        # pip is used to install simglibpy when Numpy is not yet present in
        # the system.
        ...
        if sys.version_info < (3, 6):
            raise RuntimeError(
                "simglibpy requires Python 3.6 or later. The current"
